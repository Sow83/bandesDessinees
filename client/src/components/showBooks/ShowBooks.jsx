          ...
          <div className={`"col" ${books.length < 5 && "m-auto"}`} key={book.id}>
            <div className={`card h-100 border-0 card-container ${home !== true && "pt-4"}`}>
              <div className='position-relative m-auto card-container-img'>
                <Link to={`/details/${book.id}/${cleanedTitle}`}><img src={`http://localhost:8000/images/cover/${book.reference}.jpg`} className="card-img-top card-img-bottom mb-4 card-img " alt="..." style={{ width: "188px", height: "277px" }} /></Link>
              </div>
