...
const Basket = ({cartItems, AddtoCart, removeToCart, removeItemCart}) => {
            ...
            cartItems.map((item) =>{
              return (
                <tr key={item.id} className='align-middle'>
                  <td>
                    <img className="basket-button-delete me-5" src="http://localhost:8000/images/buttonDelete.svg" alt="bouton supprimer article du panier" onClick={() => removeItemCart(item)} />
                  ...
                  <td>
                    <div className='d-flex justify-content-between align-items-center' style={{width: "130px"}}>
                      <button type='button' onClick={() => removeToCart(item)} className={"border-0"}  style={{width: "50px"}} disabled={item.quantity === 1 && "disabled"}>-</button>
                        <span className='px-1'>{item.quantity}</span>
                      <button type='button' onClick={() => AddtoCart(item)} className="border-0 " style={{width: "50px"}} disabled={item.quantity === 15 && "disabled"}>+</button>
                    </div>
                  </td>
